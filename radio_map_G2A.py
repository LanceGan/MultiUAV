...
if D == 4: 
    BS_loc=np.array([[1.503,0.827,0.025],[2.698,0.825,0.025],[0.743,2.036,0.025],[2.001,2.036,0.025],[3.247,2.036,0.025],[1.510,3.248,0.025],[2.702,3.248,0.025]]) # 4kmx4km area, 7 BSs
elif D ==5:
    BS_loc=np.array([[1.879, 1.034, 0.025],[3.373, 1.031, 0.025],[0.929, 2.545, 0.025],[2.501, 2.545, 0.025],[4.059, 2.545, 0.025],[1.888, 4.060, 0.025],[3.378, 4.060, 0.025]])
elif D ==6:
    BS_loc=np.array([[2.255, 1.241, 0.025],[4.047, 1.238, 0.025],[1.115, 3.054, 0.025],[3.002, 3.054, 0.025],[4.871, 3.054, 0.025],[2.265, 4.872, 0.025],[4.053, 4.872, 0.025]])
BS_thetaD = 120  # The downtile angle in degree [0, 180]
PB = 0.1 # BS传输功率
Fc = 2  # Operating Frequency in GHz
SIR_THRESHOLD=5 #SIR threshold in dB for outage
...
def getPointOutageMatrix(PointLoc,SIR_th):
    ...
    for i in range(len(BS_loc)):
        ...
        for sector in range(Sec_num):
            ...
            #--------------------主要改这一项-----------------------#
            SIR=SignalFromThisSector/(TotalPowerAllSector-SignalFromThisSector+10 ** ((-140 - 30) / 10)) # dB
...
if __name__ == '__main__':
    ...
    plt.savefig('results/figs/radiomap/OutageMapActualSINR_%dkm.png'%D, dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('x (meter)', fontsize=14)
    plt.ylabel('y (meter)', fontsize=14)
    plt.savefig('results/figs/radiomap/OutageMapActual_%dkm.png'%D, dpi=300, bbox_inches='tight')
    plt.show()


    np.savez('results/datas/radiomap/Radio_datas_G2A_%dkm'%D, OutageMapActual, OutageMapActualSINR ,X_vec, Y_vec)