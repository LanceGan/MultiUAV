...
def getPointOutageMatrix(PointLoc,SIR_th):
    ...
    for i in range(len(BS_loc)):
        ...
        for sector in range(Sec_num):
            ...
            #--------------------主要改这一项-----------------------# 
            SIR=SignalFromThisSector/(10 ** ((-100 - 30) / 10)) # dB
...
if __name__ == '__main__':
    ...
    plt.savefig('results/figs/radiomap/OutageMapActualSINR_A2G_%dkm.png'%D, dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('x (meter)', fontsize=14)
    plt.ylabel('y (meter)', fontsize=14)
    plt.savefig('results/figs/radiomap/OutageMapActual_A2G_%dkm.png'%D, dpi=300, bbox_inches='tight')
    plt.show()
    
    
    np.savez('results/datas/radiomap/Radio_datas_A2G_%dkm'%D, OutageMapActual, OutageMapActualSINR ,X_vec, Y_vec)