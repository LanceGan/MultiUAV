...
class Actor(nn.Module):
	def __init__(self, state_dim, action_dim, net_width, maxaction):
		...
		self.l1 = nn.Linear(state_dim, net_width) #  set model net width
	...
	def forward(self, state):
		...
		#loc = self.fc_loc_sprend(state[:, -1:]) # 位置
...
class Q_Critic(nn.Module):
	def __init__(self, state_dim, action_dim, net_width):
		...
		#self.fc_t_sprend = nn.Linear(1, user_num) # 信息素
		#self.fc_loc_sprend = nn.Linear(2, user_num) # 位置
		self.l1 = nn.Linear(state_dim + action_dim, net_width) #set actor net width
		# Q1 architecture
		self.l2 = nn.Linear(net_width, net_width)
		self.l3 = nn.Linear(net_width, net_width)
	...
	def forward(self, state, action):
		...
		#loc2 = self.fc_loc_sprend2(state[:, :])
		sa2 = torch.cat([state,action], 1)

		q2 = F.relu(self.l5(sa2))
